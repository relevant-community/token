...
contract('PreInflationaryToken', accounts => {
  ...
  it('Calculates and premints the total inflation rewards', async () => {
    retContractBalance = await preInflationaryToken.balanceOf(
      preInflationaryToken.address
    );
    expect(retContractBalance.toNumber()).to.equal(totalInflationRewards);
  });
  ...
  it('Releases rewards into buckets over time and transfers devFund to devFundAddress', async () => {
    ...
    retDevFundBalance = await preInflationaryToken.balanceOf(
      testDevFundAddress
    );
