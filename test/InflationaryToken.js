...
contract('InflationaryToken', accounts => {
  ...
  it('Returns expected parameters on initialization', async () => {
    ...
    expect(retInflationRate.toNumber()).to.equal(testInflationRate);
  ...
  it('Allows minting the initial supply', async () => {
    ...
    expect(retBalanceDistributor.toNumber()).to.equal(testInitialSupply);
  });
  ...
  it('Allows initializing the round', async () => {
    ...
    expect(retCurrentRound.toNumber()).to.equal(0);
    ...
    retCurrentRound = await inflationaryToken.currentRound();
    expect(retCurrentRound.toNumber()).to.equal(1);
  });
  ...
  it('Calculates the current inflation correctly', async () => {
    ...
    expect(retCurrentMintableTokens.toNumber()).to.equal(
      (testInitialSupply * testInflationRate) / ppm
    );
  });
  ...
  it('Allows minting the current inflation', async () => {
    ...
    expect(balanceDistributor.toNumber()).to.equal(
      testInitialSupply + retCurrentMintableTokens.toNumber()
    );
  });
});
